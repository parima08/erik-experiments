...
(function () {
  ...
  function init() {
    ...
    interact(".drag-element").draggable({
      ...
      onend: function (event) {
        if (
          !classie.has(event.target, "drag-element--dropped") &&
          !classie.has(event.target, "drag-element--dropped-text")
        ) {
    ...
    interact(".paint-area").dropzone({
      ...
      ondrop: function (event) {
        ...
        var onEndTransCallbackFn = function (ev) {
          ...
          if (type === "area") {
            paintArea(
              event.dragEvent,
              event.target,
              draggableElement.getAttribute("data-color")
            );
          }
          setTimeout(
            function () {
        ...
        draggableElement
          .querySelector(".drop")
          .addEventListener(transEndEventName, onEndTransCallbackFn);
      },
    ...
    // reset colors
    document
      .querySelector("button.reset-button")
  ...
  function paintArea(ev, el, color) {
    ...
    if (type === "area") {
      ...
      g.setAttributeNS(
        ...
        "transform",
        "translate(" +
          Number(ev.pageX - getOffset(el).left) +
          ", " +
          Number(ev.pageY - getOffset(el).top) +
          ")"
      );
  ...
  function resetColors() {
    [].slice
      .call(document.querySelectorAll(".paint-area"))
      .forEach(function (el) {
        el.style[
          classie.has(el, "paint-area--text") ? "color" : "background-color"
        ] = "";
      });
  }
